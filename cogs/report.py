...
class Report(commands.Cog):
    ...
    def store_data(self, user, answers):
        ...
        all_reports = sheet.get_all_values(
    ...
    def hv_embed(self, user, answers):
        ...
        embed = discord.Embed(
            title=f"Bug Report-{sl}", description=f"A bug reported by <@{user_id}>\n\n**----- User information -----**\n**User**: {_user}\n**Name:** {user_name}\n**User ID:** {user_id}", color=0xFDDD0B)

        embed.add_field(name="Bug Found in?", value=f"{bug_in}", inline=False)

        if bug_in != "App Crash":
            bug_location = answers["location"]  # questions 2.1
            embed.add_field(name="Bug Location?",
                            value=f"{bug_location}", inline=False)

        if "bug_details" in answers.keys():
            bug_details = answers["bug_details"]
            embed.add_field(name="Extra Details",
                            value=f"{bug_details}", inline=False)
        else:
            embed.add_field(name="Extra Details",
                            value=f"Not Provided!!", inline=False)

        embed.add_field(name="How to reproduce the bug/issue?",
                        value=f"{live_bug}", inline=False)
        if screenshots.strip() != "":
            embed.add_field(name="Screenshots",
                            value=f"{screenshots}", inline=False)
    ...
    def full_report_embed(self, user, answers):
        ...
        device = user["device"]
        embed_1 = embed.copy()
        embed_1.insert_field_at(0, name="----- Device information -----",
                                value=f"**Operating system:** {platform}\n{device}\n----------", inline=False)
        if "app_crash_details" in answers:
            crash_details = answers["app_crash_details"]  # app crash details
            embed_1.insert_field_at(
                2, name="App Crash Details", value=f"{crash_details}", inline=False)
        else:
            embed_1.insert_field_at(
                2, name="App Crash Details", value=f"Not Provided!!", inline=False)
            ...
            try:
                ...
                if option_value == "cancel":
                    ...
                    try:
                        ...
                        # store user name
                        user_info["name"] = f"{ctx.author.name}"
                        user_info["id"] = f"{ctx.author.id}"  # store user id
                        # store user tag
                        user_info["tag"] = f"{ctx.author.discriminator}"

                        option_value = platform_select.component.id
                        await platform_msg.edit(components=[])
                        # store user operating system name
                        user_info["platform"] = "Android" if option_value == "android" else "iOS"
                        ...
                        try:
                            ...
                            user_info["device"] = device_info.content.replace("Device Name:", "**Device Name:**").replace("Model Number:", "**Model Number:**").replace("Android Version:", "**Android Version:**").replace(
                                "iOS Version:", "**iOS Version:**").replace("App Version:", "**App Version:**").replace("Ram (Optional):", "**Ram:**").replace("Rom (Optional):", "**Rom:**")  # store user device information
                            ...
                            try:
                                ...
                                # store bug location
                                answers["bug_in"] = option.component[0].label
                                await options_msg.edit(components=[])
                                ...
                                if bug_found:
                                    ...
                                    try:
                                        ...
                                        if has_ss:
                                            ...
                                            # preview
                                            preview_embed = self.full_report_embed(
                                                user_info, answers)
                                            ...
                                            try:
                                                ...
                                                if value == "no":
                                                    ...
                                                    normal_em = self.hv_embed(
                                                        user_info, answers)
                                                    main_em = self.full_report_embed(
                                                        user_info, answers)
                                                    if answers["bug_in"] == "Content":
                                                        content_ch = [
                                                            557639363807150092, 826686813103587341]
                                                        ...
                                                        bug_ch = [
                                                            634995781404983296, 826683797377384448]
                                                        for i in bug_ch:
                                                            channel = await self.bot.fetch_channel(i)
                                                            await channel.send(embed=normal_em)
