    ...
    async def compile(self, ctx):
        ...
        ### loading gif reaction ####
            ...
            if not lang_error and not code_error:
                ...
                if compile_response:
                    ...
                    if compile_response.status_code == 200:
                        ...
                        compile_embed = discord.Embed(
                            title=f"{lang.capitalize()} Compilation Results", color=0xdbca32, timestamp=time.now())
                        if len(compile_code.get('stderr')) != 0:
                            ...
                            if compile_response.json().get("compile"):
                                ...
                                status_code = compiler.get(
                                    "code") if compiler.get("code") else None
                            else:
                        ...
                        if self.need_edit and ctx.author.id in self.data:
                            ...
                            compile_embed.add_field(name=f"Status Code", value=f"```\nPrograme finished with status code {status_code}\n```") if (
                                status_code or status_code == 0) else None
                            compile_embed.add_field(
                                name=f"Output", value=f"```\n{output}\n```") if output else None
                            compile_embed.add_field(
                                name=f"Error", value=f"```\n{error}\n```") if error else None
            ...
            ########## Error Part ############
                        ...
                        await ctx.message.clear_reactions()
                        await ctx.message.add_reaction("<:wrong:846424916404207636>")
                        msg = discord.Embed(title=":warning: Compiler Error :warning:",
                ...
                await ctx.message.clear_reactions()
                await ctx.message.add_reaction("<:wrong:846424916404207636>")
                msg = discord.Embed(title=":warning: Compiler Error [Code block not found] :warning:",
                                    description="Write your code inside code block.\nUse **```** before and after your code.\n__For Example:__\n", color=0xC70039)
            ...
            for prefix in prefixes:
                if after.content.lower().startswith(f'{prefix}compile'):
                    after.content = after.content.replace(
                        f'{prefix}compile', f'{prefix}compile')
                    self.need_edit = True
...
# all supported programming language
    ...
    async def lang(self, ctx):
        ...
        em_msg = discord.Embed(title="Compiler Languages", description='\n'.join(
            i for i in message_content[start: end]), timestamp=time.now(), color=0xffdf08)
       ...
       ####### Paginator #########
        ...
        while True:
            ...
            if reaction.emoji == emojis[1] and page != pages:
                ...
                edit_em_msg.set_footer(
                    text=f"Page: {page}/{pages} | Programming Hero")
