        ...
        if get_guild != None:
            ...
            if welcome_text == None:
                ...
                data = {"guild_id": ctx.guild.id,
                        "welcome_channel": channel.id, "welcome_msg": welcome_text}
                tb.insert_one(data)
        ...
        if welcome_msg == None:
            ...
            if get_guild != None:
                ...
                if old_welcome_msg != None:
                    tb.update_one({"guild_id": ctx.guild.id}, {
                ...
                tb.insert_one({"guild_id": ctx.guild.id,
...
#		welcomeChannel = c_fibu.execute("select welcome_channel from guild_data where guild_id=?",(member.guild.id,)).fetchone()
        ...
        try:
            ...
            if welcome_msg != None:
                # finding all attributes that are between { and } and removing the curly braces
                find_attributes = re.findall(
                    "{[a-z]+\.[a-z]+}"[1:-1], welcome_msg)
                for attribute in find_attributes:
                    ...
                    if first == "user" or first == "member":
                        if last == "name":
                            ...
                            welcome_msg = welcome_msg.replace(
                                "{"+first+".mention}", member.mention)
                        elif last == "user":
                            welcome_msg = welcome_msg.replace(
                                "{"+first+".user}", member)
                        elif last == "guild":
                            welcome_msg = welcome_msg.replace(
                                "{"+first+".guild}", member.guild.name)
                    # server
                    elif first == "server" or first == "guild":
                        if last == "name":
                            welcome_msg = welcome_msg.replace(
                                "{"+first+".name}", member.guild.name)
                        elif last == "id":
                            welcome_msg = welcome_msg.replace(
                                "{"+first+".id}", member.guild.id)
            # End
            else:
            ...
            if welcomeChannel is None:
                ...
                if sys_channel is None:
                    ...
                    await sys_channel.send(welcome_msg)
            else:
                welcomeChannel = get(member.guild.channels,
                                     id=welcomeChannel["welcome_channel"])
                if welcomeChannel is None:
                    pass
                else:
