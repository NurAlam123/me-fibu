...
# server info
    ...
    async def serverinfo(self, ctx):
        ...
        guild_roles = "\n".join(
            f"{i}. {j}" for i, j in enumerate(guild_roles[::-1], 1))
        ...
        info_em = discord.Embed(
            title="Server Information", color=random.choice(self.colors))
        info_em.add_field(name="Name", value=f"{guild_name}", inline=False)
        info_em.add_field(
            name="Guild ID", value=f"```\n{guild_id}\n```", inline=False)
        info_em.add_field(name="Owner", value=f"{guild_owner}", inline=False)
        info_em.add_field(
            name="Owner ID", value=f"```\n{guild_owner_id}\n```", inline=False)
        info_em.add_field(name="Server Created At",
                          value=f"```\n{guild_created_at}\n```")
        info_em.add_field(name="Region", value=f"{guild_region}", inline=False)
        if guild_description:
            info_em.add_field(name="Guild Description",
                              value=f"{guild_description}", inline=False)
        info_em.add_field(
            name=f"Members [{members}]", value=f"```\nHumans: {humans}\nBots: {bots}\n--------------------\nOnline: {online}\nOffline: {offline}\nIdle: {idle}\nDND: {dnd}\n```", inline=False)
        info_em.add_field(name="Channels and Categories",
...
# our team **need more improvement**
    ...
    async def _team(self, ctx):
        ...
        msg = discord.Embed(title="Developer information",
                            description="Here are my developers:", color=0xffdf08, timestamp=time.now())
        msg.add_field(
            name=f"Nur Alam [`{team[0]}`]", value="Worked on my designing and development.", inline=False)
        msg.add_field(name=f"Tamim Vaiya [`{team[1]}`]",
                      value="Gave suggestions to my developers.", inline=False)
        msg.add_field(
            name=f"Rishikesh [`{team[2]}`]", value="Worked on my development.", inline=False)
        msg.add_field(name=f"Soren_Blank [`{team[3]}`]",
                      value="Worked on my development.", inline=False)
        msg.add_field(name=f"Shajedul Karim [`{team[4]}`]",
                      value="Secret person behind this secret project!!")
        msg.set_author(name=f"{self.bot.user.name}",
                       url="https://www.programming-hero.com/", icon_url=f"{self.bot.user.avatar_url}")
...
# user info
    ...
    async def userinfo(self, ctx, member: discord.Member = None):
        ...
        if member.id == self.bot.user.id:
            ...
            #### Embed Part ####
            info_em = discord.Embed(
                title=f"{suf}User Information", color=random.choice(self.colors))
            info_em.add_field(
                name="Name", value=f"```\n{user_name}\n```", inline=True)
            info_em.add_field(
                name="Tag", value=f"```\n#{user_tag}\n```", inline=True)
            ...
            if user_nickname:
                info_em.add_field(
                    name="Nickname", value=f"```\n{user_nickname}\n```", inline=False)
            info_em.add_field(
                name="Status", value=f"{status_emoji[user_status]} ─ **{status}**", inline=False)
            info_em.add_field(
                name=f"Joined {guild.name} at", value=f"```\n{joined_guild}\n```", inline=False)
            info_em.add_field(name="Account Created at",
                              value=f"```\n{created_acc}\n```", inline=False)
            info_em.add_field(name="Badges", value=user_badges,
           ...
           #### challenge"s information ####
            ...
            ########
            info_em.add_field(
                name=f"Roles [{len(roles)}]", value=f"```\n{roles_format}\n```", inline=False)
...
# avatar
    ...
    async def _av(self, ctx, member: discord.Member = None):
        ...
        avatar = discord.Embed(
            title="Avatar", color=0xffdf08, timestamp=time.now())
...
# member count
    ...
    async def count(self, ctx, arg=None):
        ...
        if arg == None:
            ...
            msg = discord.Embed(
                title="Members", color=0xffdf08, timestamp=time.now())
            msg.add_field(name="Server Name",
