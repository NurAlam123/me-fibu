...
# covid19
    ...
    async def covid(self, ctx, *, country_name=None):
        ...
        if country_name != None:
            ...
            msg.add_field(name="Total Deaths",
                          value=f"{world['TotalDeaths']:,}")
            msg.add_field(name="Total Recovered",
                          value=f"{world['TotalRecovered']:,}")
            msg.set_thumbnail(url="https://is.gd/TTTW35")
            msg.set_author(name=f"{self.client.user.name}",
                           url="https://www.programming-hero.com/", icon_url=f"{self.client.user.avatar_url}")
            msg.set_footer(text="Programming Hero ")
            await ctx.send(embed=msg)
            is_done = True
        if not is_done:
