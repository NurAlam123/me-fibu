        ...
        if ":" in message.content:
            ...
            for emote in emotes:
                ...
                if emoji:
                    if emoji.animated:
                        ...
                        em = True
                    if not emoji.animated and emoji not in message.guild.emojis:
                        em = True
                        get_emoji = f"<:{emoji.name}:{emoji.id}>"
                        user_message = user_message.replace(emote, get_emoji)
            ...
            if em:
                ...
                if message.reference:
                    ...
                    em = discord.Embed(title= f"{message.author.mention} replied {get_message.author.mention}'s message", description= f"[Jump to that message!]({get_message.jump_url})")
