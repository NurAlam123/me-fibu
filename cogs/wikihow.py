...
# Search article
    ...
    async def how(self, ctx, *, query):
        ...
        if search == []:
            ...
            msg = discord.Embed(
                title=f"{search[0]['title']}", description=f"{main} ...[more info]({url})", color=0xffdf08, timestamp=time.now())
            try:
...
# Random article
    ...
    async def randomHow(self, ctx):
        ...
        main = whapi.parse_intro(article_id)[:500]
        msg = discord.Embed(
            title=f"{article_title}", description=f"{main} ...[more info]({article_url})", color=0xffdf08, timestamp=time.now())
        try:
            all_step = whapi.parse_steps(article_id)
            steps = []
            for i, step in enumerate(all_step, 1):
